...
def save_message_to_db(db: Session, conversation_id: int, role: str, message: str, user_id: int = None):
...
def detect_user_facts(db: Session, user_id: int, message: str):
    if not user_id:
        return
    ...
    if "my name is " in lower_msg:
        detected_name = message[lower_msg.find("my name is ") + 11:].strip()
    elif "je m'appelle " in lower_msg:
        detected_name = message[lower_msg.find("je m'appelle ") + 13:].strip()
    elif "mon nom est " in lower_msg:
        detected_name = message[lower_msg.find("mon nom est ") + 12:].strip()
    ...
    if detected_name:
        ...
        existing = db.query(models.UserMemory).filter(
            ...
            models.UserMemory.memory_key == "name"
        ).first()
        
        if existing:
            existing.memory_value = detected_name
        else:
            new_memory = models.UserMemory(
                user_id=user_id,
                memory_key="name",
                memory_value=detected_name
            )
            db.add(new_memory)
        db.commit()

def get_fallback_response(message: str, last_error_type: str) -> str:
    ...
    if last_error_type in ["QUOTA", "UNKNOWN", "TEXT_ACCESS_ERROR"]:
        try:
            ...
            if found:
                ...
                if len(desc) > 300:
                    desc = desc[:300] + "..."
                return f"Since I'm in offline mode, I retrieved this from my local database:\n\n**{target.get('name')}** ({target.get('city')})\n\n{desc}\n\n*This is an automated fallback response.*"
            
            city_names = sorted(list(set(d.get('city', 'Unknown') for d in dests)))
            cities_str = ", ".join(city_names[:10])
...
def generate_chat_response(db: Session, message: str, session_id: str = None, user_id: int = None) -> tuple[str, str]:
    if not client:
    ...
    for model_name in models_to_try:
        try:
            ...
            # Add recent history
            for msg in history_records[-11:-1]: # Last 10 before the current one
